...
const i18n = {
  fr: {
    ...
    cars_title: "Notre Flotte Premium",
    cars_sub: "Faites défiler la galerie, choisissez votre voiture puis réservez en quelques secondes.",
  ...
  ar: {
    ...
    cars_title: "أسطولنا المميز",
    cars_sub: "مرر داخل المعرض، اختر السيارة المناسبة ثم احجز بسرعة.",
  ...
  en: {
    ...
    cars_title: "Our Premium Fleet",
    cars_sub: "Slide through the gallery, choose your car and book in seconds.",
  ...
  nl: {
    ...
    cars_title: "Onze Premium Vloot",
    cars_sub: "Blader door de galerij, kies uw auto en reserveer in enkele seconden.",
  ...
  de: {
    ...
    cars_title: "Unsere Premium-Flotte",
    cars_sub: "Blättern Sie durch die Galerie, wählen Sie Ihr Auto und buchen Sie in wenigen Sekunden.",
...
function getWrappedDelta(index, active, total){
...
function startCarouselAutoplay(){
  ...
  if (!carouselElements || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  carouselTimer = window.setInterval(() => {
    goToCarouselIndex(activeCarIndex + 1, false);
  }, 3800);
}
...
function syncSelectedCar(index){
  activeCarIndex = (index + cars.length) % cars.length;
  carSelect.value = String(activeCarIndex);
  updateCarPreview();
...
function updateCarouselPositions(){
  ...
  cards.forEach((card, index) => {
    ...
    let transform = 'translate(-50%, -50%) scale(.35)';
    let opacity = '0';
    let zIndex = '1';
    let filter = 'blur(3px) saturate(.55) brightness(.45)';
    ...
    if (abs === 0) {
      ...
      card.dataset.state = delta < 0 ? 'far-left' : 'far-right';
    } else {
  ...
  dots.forEach((dot, index) => {
    dot.classList.toggle('is-active', index === activeCarIndex);
    dot.setAttribute('aria-current', index === activeCarIndex ? 'true' : 'false');
  });
}

function goToCarouselIndex(index, userInitiated = true){
  syncSelectedCar(index);
  if (userInitiated) restartCarouselAutoplay();
}
...
function renderCarsGrid(){
  ...
  grid.innerHTML = `
    <div class="fleetCarousel" aria-label="${dict.cars_title}">
      <button class="fleetCarousel__nav fleetCarousel__nav--prev" type="button" aria-label="Previous car">‹</button>
      <div class="fleetCarousel__viewport">
        <div class="fleetCarousel__track"></div>
      </div>
      <button class="fleetCarousel__nav fleetCarousel__nav--next" type="button" aria-label="Next car">›</button>
      <div class="fleetCarousel__dots" aria-label="Carousel pagination"></div>
    </div>
  `;

  const viewport = grid.querySelector('.fleetCarousel__viewport');
  const track = grid.querySelector('.fleetCarousel__track');
  const dotsWrap = grid.querySelector('.fleetCarousel__dots');
  ...
  cars.forEach((car, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="fleetCard__content">
        <div class="fleetCard__brand">YAMALI RENT CAR</div>
        <div class="fleetCard__model">${getCarName(car)}</div>
        <div class="fleetCard__meta">${getCarMetaText(car)}</div>
    ...
    dot.type = 'button';
    dot.className = 'fleetCarousel__dot';
    dot.setAttribute('aria-label', `${dict.car_btn} ${getCarName(car)}`);
    dot.addEventListener('click', () => goToCarouselIndex(index));
    dotsWrap.appendChild(dot);
  });

  carouselElements = {
    root: grid.querySelector('.fleetCarousel'),
    viewport,
    track,
  ...
  grid.querySelector('.fleetCarousel__nav--prev').addEventListener('click', () => goToCarouselIndex(activeCarIndex - 1));
  grid.querySelector('.fleetCarousel__nav--next').addEventListener('click', () => goToCarouselIndex(activeCarIndex + 1));

  grid.querySelectorAll('[data-car-index]').forEach((link) => {
    link.addEventListener('click', (event) => {
      const idx = Number(event.currentTarget.getAttribute('data-car-index') || '0');
      carSelect.value = String(idx);
      goToCarouselIndex(idx);
    });
  });
  ...
  viewport.addEventListener('pointerup', (event) => {
    ...
    if (Math.abs(deltaX) > 40) {
      goToCarouselIndex(activeCarIndex + (deltaX < 0 ? 1 : -1));
    }
    pointerStartX = null;
  });
  ...
  carouselElements.root.addEventListener('mouseenter', stopCarouselAutoplay);
  carouselElements.root.addEventListener('mouseleave', startCarouselAutoplay);
  carouselElements.root.addEventListener('focusin', stopCarouselAutoplay);
  carouselElements.root.addEventListener('focusout', (event) => {
    if (!carouselElements.root.contains(event.relatedTarget)) startCarouselAutoplay();
  });

  activeCarIndex = Math.min(activeCarIndex, cars.length - 1);
  updateCarouselPositions();
  startCarouselAutoplay();
}
...
carSelect.addEventListener("change", () => {
  const nextIndex = parseInt(carSelect.value || "0", 10);
  activeCarIndex = Number.isNaN(nextIndex) ? 0 : nextIndex;
  updateCarPreview();
  updateCarouselPositions();
